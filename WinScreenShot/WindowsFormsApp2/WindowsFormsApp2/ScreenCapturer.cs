...
namespace WindowsFormsApp2
{
    ...
    public static class ScreenCapturer
    {
        ...
        {
            //找调用不同程序的intptr的程序，使得对应list产生程序
        ...
        /// <param name="handle">hWnd (handle) of the window to capture</param>
        public static Bitmap Capture(IntPtr handle)//pointer
        {
            Rectangle bounds;
            var rect = new Rect();
            GetWindowRect(handle, ref rect);
            bounds = new Rectangle(rect.Left, rect.Top, rect.Right - rect.Left, rect.Bottom - rect.Top);
            CursorPosition = new Point(CursorPosition.X - rect.Left, CursorPosition.Y - rect.Top);
        ...
        static void ImageSave(string filename, ImageFormat format, Image image)
        {
            ...
            if (!filename.Contains(@"\"))
                filename = Path.Combine(Environment.GetEnvironmentVariable("TEMP") ?? @"D:\Temp", filename);
